...
def burn_subtitle_to_video(input_video=None, subtitle_file=None, output_file=None):
    ...
    if input_video is None:
        ...
        for file in os.listdir(output_dir):
            if file.endswith(tuple(video_extensions)) and not file.startswith("output_") and not file.endswith("_360p.mp4"):
    ...
    if output_file is None:
        ...
        output_file = f"output/{base_name}_with_subtitles.mkv"
    ...
    # 使用ffmpeg烧录字幕
    cmd = [
        'ffmpeg', '-hwaccel', 'auto', '-i', input_video, 
